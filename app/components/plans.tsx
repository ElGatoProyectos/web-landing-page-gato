...
const plans = (prop: Props) => {
  ...
  return (
    <div className="w-full bg-white" id="planes">
      <div className="w-full lg:px-16 px-8  max-w-[1440px] mx-auto py-16 md:py-20 ">
        <h2 className="text-stone-800 text-4xl md:text-6xl font-semibold mb-5 text-center">
          Conoce nuestros <span className="text-[#4608AD]">paquetes</span> para
          emprendedores
        </h2>
        ...
        <div className="px-6 w-full mb-14 text-center">
          ...
          />
          <p className="text-center mt-2 lg:mt-7 text-stone-600">
            Personaliza tu plan con nosotros.{" "}
            <button
              onClick={prop.openPopup}
              className="text-[#4608AD] font-semibold"
            >
        ...
        {selectedPlan && (
          ...
          >
            ...
            <h2 className="text-stone-600 text-center text-4xl mb-7 font-bold capitalize">
              {selectedPlan.plan_name}
            </h2>
            <div className="flex flex-col md:flex-row gap-20 lg:gap-28 justify-between">
              <div className="md:w-1/2 2xl:w-2/4 2xl:pr-10">
                <div className="text-[#4608AD] font-bold text-xl mb-7">
                  <p>
                    {selectedPlan.short_description}: S/. {selectedPlan.price}
                    .00
                  </p>
                </div>
                <p className="text-stone-600 text-center w-full ">
                  {selectedPlan.long_description}
                </p>

                <h3 className={`text-xl font-bold mb-2 text-[#4608AD]`}>
                  Incluye
                </h3>
                  ...
                  {selectedPlan.items.length > 0 &&
                    ...
                    selectedPlan.items[0].sub_items.map(
                      (detail: any, index: number) => (
                        <li
                          key={index}
                          className={`flex justify-start items-start text-stone-600 mb-3`}
                        >
                          <div>
                            <svg
              ...
              <div className="relative md:w-1/2 2xl:w-2/5 bg-[#F4F5FF] px-8 pt-8 rounded-lg shadow-lg flex flex-col justify-between lg:justify-start items-center lg:items-start">
                <h3 className="text-[#4608AD] font-medium text-3xl ">
                ...
                <button
                  onClick={prop.openPopup}
                  className="border border-[#4608AD] bg-transparent text-[#4608AD] rounded-full text-xl font-medium px-4 py-2 mt-7 hover:bg-[#4608AD] hover:text-white transition-all duration-300"
                >
                  Cotizar
                </button>
                <img
